...
export default class NoteStatisticController {
    ...
    public getWordsPerDot(): number {
        return this.plugin.database.setting.wordsPerDot;
    }

    public setWordsPerDot(wordsPerDot: number): void {
        this.plugin.database.setting.wordsPerDot = wordsPerDot;
    }

    public getDotUpperLimit(): number {
        return this.plugin.database.setting.dotUpperLimit;
    }

    public setDotUpperLimit(dotUpperLimit: number): void {
        this.plugin.database.setting.dotUpperLimit = dotUpperLimit;
    }

    public getTodoAnnotationMode(): TodoAnnotationMode {
        return this.plugin.database.setting.todoAnnotationMode;
    }

    public setTodoAnnotationMode(todoAnnotationMode: TodoAnnotationMode): void {
        this.plugin.database.setting.todoAnnotationMode = todoAnnotationMode;
    }
    ...
    public addTaskByDateAndNoteType(date: DateTime, noteType: NoteType): void {
        setTimeout(() => {
            void this.executeStaticByDateAndNoteType(date, noteType);
        }, 0);
    }
    ...
    public addTaskByFile(file: TAbstractFile | null): void {
        setTimeout(() => {
            void this.executeStaticByFile(file);
        }, 0);
    }
    ...
    private async executeStaticByFile(file: TAbstractFile | null): Promise<void> {
        ...
        // 统计待办
        if (todoAnnotationMode === TodoAnnotationMode.NONE) {
            newNoteStatistic.hasUnfinishedTasks = false;
        }
        else {
            const totalTodos = NoteStatisticController.countTodos(content);
            newNoteStatistic.hasUnfinishedTasks = totalTodos !== 0;
        }
    ...
    private static countWords(text: string): number {
        const result = text.match(NoteStatisticController.PATTERN_FOR_COUNTING_WORDS);
        return result ? result.length : 0;
    }

    private static countTodos(text: string): number {
